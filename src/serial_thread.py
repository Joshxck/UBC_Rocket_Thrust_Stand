...
PACKET_SIZE = 27  # was 23

# Added I (uint32) for timestamp at the end
PACKET_FORMAT = '<BBiiHHHHHI'  # little-endian
...
class SerialWorker(QThread):
    ...
    def parse_packet(self, raw: bytes) -> dict | None:
        ...
        # Verify checksum (XOR of bytes 2..25, was 2..21)
        chk = 0
        for b in raw[2:26]:
            chk ^= b
        if chk != raw[26]:  # was raw[22]
            return None

        # Unpack payload (skip 2 header bytes)
        thr1, thr2, lc1, lc2, rpm1, rpm2, c1, c2, c3, timestamp_ms = struct.unpack_from(
            PACKET_FORMAT, raw, 2
        )

        return {
            'throttle1':     round(thr1 / 255 * 100, 1),
            ...
            'timestamp_ms':  timestamp_ms,   # ms since ESP32 boot
        }
